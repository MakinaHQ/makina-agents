...
function fish_greeting
  echo "makina/agents · autonomous coding sandbox"
end
...
def ensure_global_gitignore(workspace: Path) -> None:
    """Copy template gitignore to container and configure git to use it.

    Uses --system config since ~/.gitconfig is mounted read-only from host.
    """
    source = workspace / ".devcontainer" / ".gitignore_global"
    if not source.exists():
        log("no .gitignore_global template found in .devcontainer")
        return
    ...
    if not excludes_path.exists():
        ...
        log(f"copied gitignore template to {excludes_path}")
    else:
        log(f"gitignore already exists at {excludes_path}")

    # Use --system since ~/.gitconfig is read-only (mounted from host)
    run_git(["config", "--system", "core.excludesfile", str(excludes_path)], workspace)
    log(f"configured git system config to use {excludes_path}")
...
def main() -> None:
    ...
    ensure_dir_ownership(Path.home() / ".foundry")
    ensure_fish_history()
